                ...
                <div class="row">
                    ...
                    <div class="col-md-6 col-lg-4 mb-4 mb-md-0">
                        ...
                        <div class="block-23 mb-3">
                            <ul color="white"><?php pms_displayWidgets("footer_col_3", $pms_page_id); ?>
                            </ul>
                        </div>
                    </div>
                </div>
    ...
    <div id="footerRights">
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-6">
                    <p class="text-right">
                        <a href="<?php echo DOCBASE; ?>feed/" target="_blank"
                            title="<?php echo $pms_texts['RSS_FEED']; ?>"><i class="fas fa-fw fa-rss"></i></a>
                        <?php
                        foreach ($pms_menus['footer'] as $nav_id => $nav) { ?>
                            <a href="<?php echo $nav['href']; ?>"
                                title="<?php echo $nav['title']; ?>"><?php echo $nav['name']; ?></a>
                            &nbsp;&nbsp;
                            <?php
                        } ?>
                    </p>
...
if ($result_popup !== false && $pms_db->last_row_count() > 0) {
    ...
    if (!isset($_SESSION['popup_' . $id_popup])) {
        ...
        <div id="popup-<?php echo $id_popup; ?>" class="white-popup-block mfp-hide" <?php if (!empty($popup_bg))
               echo ' style="background-color:' . $popup_bg . ';"'; ?>>
...
if (
    ...
    <div id="booking-cart" class="alert alert-dismissible">
        <form method="post" class="ajax-form">
            <a href="#" class="close sendAjaxForm"
                data-action="<?php echo pms_getFromTemplate('common/cancel_booking.php'); ?>" data-dismiss="alert"
            ...
            <a href="<?php echo DOCBASE . $pms_sys_pages[$step]['alias']; ?>"
                class="alert-link"><?php echo $pms_texts['COMPLETE_YOUR_BOOKING']; ?></a><br>
            <small><?php echo gmstrftime(PMS_DATE_FORMAT, $_SESSION['book']['from_date']); ?> <i
                    class="fas fa-fw fa-arrow-right"></i>
                <?php echo gmstrftime(PMS_DATE_FORMAT, $_SESSION['book']['to_date']); ?></small><br>
            <?php if (isset($_SESSION['book']['num_rooms']))
                echo $_SESSION['book']['num_rooms'] . ' ' . pms_getAltText($pms_texts['ROOM'], $pms_texts['ROOMS'], $_SESSION['book']['num_rooms']); ?>
            <b><?php if ($_SESSION['book']['total'] > 0)
                echo ' - ' . pms_formatPrice($_SESSION['book']['total']); ?></b>
...
<script>
    Modernizr.load({
        load: [
            ...
            <?php if (isset($pms_javascripts))
                foreach ($pms_javascripts as $javascript)
                    echo "'" . $javascript . "',\n"; ?>
    ...
    $(function () {
        ...
        if (isset($field_notice) && !empty($field_notice))
            foreach ($field_notice as $field => $notice)
                echo "$('.field-notice[rel=\"" . $field . "\"]').html('" . $notice . "').fadeIn('slow').parent().addClass('alert alert-danger');\n"; ?>
    });
</script>
